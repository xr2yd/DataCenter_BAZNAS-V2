...
export function PenyaluranTopNav() {
  ...
  return <header className="sticky top-0 z-40 border-b border-zinc-200 bg-white/95 backdrop-blur-xs">
    <div className="mx-auto grid h-16 max-w-[1920px] grid-cols-[1fr_auto_1fr] items-center gap-3 px-4 sm:px-6">
      ...
      <nav aria-label="Navigasi utama" className="hidden items-center gap-1 md:flex">{PENYALURAN_NAV_ITEMS.map((item) => {
      ...
      <div className="relative justify-self-end"><button type="button" onClick={() => { setProfileOpen(!profileOpen); setMenuOpen(null); }} className="flex h-11 items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50/50 py-1 pl-1.5 pr-2.5 text-left text-xs transition hover:bg-zinc-100"><span className="grid size-7 place-items-center rounded-full bg-emerald-600 text-[10px] font-bold text-white">{initials || 'AP'}</span><span className="hidden flex-col leading-none sm:flex"><strong className="max-w-[130px] truncate text-sm text-zinc-900">{userName}</strong><small className="mt-1 text-[10px] font-medium text-zinc-500">Bidang Penyaluran</small></span><ChevronDown className="size-3 text-zinc-400" /></button>{profileOpen && <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-52 rounded-2xl border border-zinc-200 bg-white p-2 shadow-xl"><div className="border-b border-zinc-100 px-3 py-2"><p className="text-xs font-bold text-zinc-900">{userName}</p><p className="mt-1 truncate text-[10px] text-zinc-500">{user?.email || 'amil@baznas.go.id'}</p></div><Link href="/penyaluran/pengaturan" onClick={() => setProfileOpen(false)} className="mt-1 flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-bold text-zinc-700 hover:bg-zinc-50"><Settings2 className="size-4" />Pengaturan</Link><button type="button" onClick={logout} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-bold text-rose-600 hover:bg-rose-50"><LogOut className="size-4" />Keluar</button></div>}</div>
    </div>
  </header>;
}
