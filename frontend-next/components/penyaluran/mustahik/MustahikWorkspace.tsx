  ...
  return <main className="min-w-0 bg-slate-50/60">
    ...
    <div className="border-b border-slate-200 bg-white px-4 py-4 md:px-5">
      ...
      <section aria-label="Ringkasan keputusan" className="mt-4 rounded-2xl border border-emerald-100 bg-[linear-gradient(110deg,#f0fdf7_0%,#ffffff_62%)] p-3 shadow-[0_10px_28px_rgba(5,150,105,0.055)] sm:p-4">
        <div className="flex flex-col items-center gap-3 text-center sm:flex-row sm:justify-center sm:gap-5"><div aria-label={`Skor kelayakan ${selected.score} dari 100`} className="shrink-0 border-b border-emerald-200 pb-2 text-emerald-800 sm:border-b-0 sm:border-r sm:pb-0 sm:pr-5"><div className="flex items-baseline justify-center gap-1"><strong className="text-4xl font-extrabold leading-none tracking-[-0.06em]">{selected.score}</strong><span className="text-sm font-extrabold text-slate-400">/100</span></div><p className="mt-1 text-[10px] font-extrabold uppercase tracking-[0.12em] text-emerald-700">Skor kelayakan</p></div><div className="min-w-0"><div className="flex flex-wrap items-center justify-center gap-2"><span className="rounded-full bg-emerald-100 px-2 py-1 text-[11px] font-extrabold tracking-wide text-emerald-700">LAYAK</span><p className="font-extrabold text-slate-950">Keputusan siap ditinjau</p></div><p className="mt-1 text-xs leading-5 text-slate-500">{selected.missingDocument ? documentLabel : 'Hasil verifikasi lengkap dan siap diproses.'}</p></div><button type="button" onClick={onDecision} aria-label="Buka panel keputusan" className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-3 text-xs font-extrabold text-white shadow-sm transition hover:bg-emerald-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-100 sm:px-4 sm:text-sm"><ClipboardCheck className="size-4" />Tinjau keputusan</button></div>
      </section>
    </div>
