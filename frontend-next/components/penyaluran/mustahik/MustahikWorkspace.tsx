  ...
  return <main className="min-w-0 bg-slate-50/60">
    ...
    <div className="border-b border-slate-200 bg-white px-4 py-4 md:px-5">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_270px] lg:items-center">
        ...
        <div className="rounded-2xl bg-slate-50 px-3.5 py-3"><div className="mb-2 flex items-center justify-between text-xs font-bold text-slate-500"><span>Kelengkapan data</span><span className="text-emerald-700">{selected.completeness}%</span></div><div className="h-2 rounded-full bg-slate-200"><div className="h-full rounded-full bg-emerald-600 transition-[width] duration-700" style={{ width: `${selected.completeness}%` }} /></div></div>
      </div>
      <section aria-label="Ringkasan keputusan" className="mt-3 overflow-hidden rounded-[20px] border border-emerald-100 bg-white shadow-[0_12px_30px_rgba(5,150,105,0.07)]">
        <div className="grid md:grid-cols-[152px_minmax(0,1fr)_auto] md:items-stretch"><div aria-label={`Skor kelayakan ${selected.score} dari 100`} className="flex min-h-28 flex-col items-center justify-center bg-emerald-800 px-5 py-4 text-center text-white"><div className="flex items-baseline justify-center gap-1"><strong className="text-4xl font-extrabold leading-none tracking-[-0.06em]">{selected.score}</strong><span className="text-sm font-extrabold text-emerald-200">/100</span></div><p className="mt-1 text-[10px] font-extrabold uppercase tracking-[0.14em] text-emerald-100">Skor kelayakan</p></div><div className="flex flex-col justify-center px-5 py-4 text-center md:text-left"><div className="flex flex-wrap items-center justify-center gap-2 md:justify-start"><span className="rounded-full bg-emerald-100 px-2 py-1 text-[11px] font-extrabold tracking-wide text-emerald-700">LAYAK</span><p className="text-base font-extrabold text-slate-950">Keputusan siap ditinjau</p></div><p className="mt-1 text-sm leading-5 text-slate-500">{selected.missingDocument ? documentLabel : 'Hasil verifikasi lengkap dan siap diproses.'}</p></div><div className="flex items-center justify-center border-t border-slate-100 bg-slate-50/70 p-3 md:border-l md:border-t-0"><button type="button" onClick={onDecision} aria-label="Buka panel keputusan" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 text-sm font-extrabold text-white shadow-sm transition hover:bg-emerald-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-100"><ClipboardCheck className="size-4" />Tinjau keputusan</button></div></div>
      </section>
    </div>
