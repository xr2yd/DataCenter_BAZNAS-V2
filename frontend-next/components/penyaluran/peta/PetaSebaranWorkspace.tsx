      ...
      <section aria-labelledby="peta-utama-title" className="space-y-3">
        <div className="flex flex-col gap-3 px-1 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div role="group" aria-label="Metrik peta" className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 transition hover:border-emerald-200 hover:bg-emerald-50">Semua program</button>
          </div>
        </div>
        ...
        <div className="grid items-stretch gap-4 lg:grid-cols-[minmax(0,1.65fr)_minmax(290px,.72fr)]">
          ...
          <aside aria-live="polite" className="flex min-h-[430px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_10px_24px_rgba(15,23,42,0.03)]">
            <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.13em] text-emerald-700">Wilayah terpilih</p><h2 className="mt-1 text-2xl font-extrabold tracking-tight">{selectedKecamatan}</h2><p className="mt-1 text-xs text-slate-500">Pembaruan data hari ini</p></div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-extrabold text-emerald-700">PRIORITAS</span></div>
            ...
            <dl className="grid grid-cols-2 gap-2 border-t border-slate-100 pt-4 text-sm"><div className="rounded-xl bg-slate-50 px-3 py-3"><dt className="text-[11px] font-bold text-slate-500">Mustahik terbantu</dt><dd className="mt-1 font-extrabold tracking-tight">{(selectedData?.totalMustahik ?? 0).toLocaleString('id-ID')} jiwa</dd></div><div className="rounded-xl bg-slate-50 px-3 py-3"><dt className="text-[11px] font-bold text-slate-500">Program dominan</dt><dd className="mt-1 font-extrabold tracking-tight">{selectedInsight.topProgram}</dd></div><div className="col-span-2 rounded-xl border border-amber-100 bg-amber-50 px-3 py-3"><dt className="text-[11px] font-extrabold uppercase tracking-wide text-amber-700">Fokus tindak lanjut · {selectedInsight.dominantAsnaf}</dt><dd className="mt-1 text-xs font-semibold leading-5 text-amber-950">{selectedInsight.priorityNote}</dd></div></dl>
            <Link href={`/penyaluran/mustahik?kecamatan=${encodeURIComponent(selectedKecamatan)}`} className="mt-auto flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 text-sm font-bold text-white transition hover:bg-emerald-800">Buka daftar mustahik <ArrowRight className="size-4" /></Link>
          </aside>
        </div>
      </section>
